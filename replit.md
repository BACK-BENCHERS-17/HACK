...
- **`GET /qr.php`** (also `/qr`, `/api/qr`) — params `api_key`, `upi`, `amount`, optional `payee`, `order_id`, `format=json|png`. Returns either JSON (with `qr_image_b64` + `qr_url` + `order_id` + `expires_at_ist`) or a raw PNG when `format=png` (perfect for `<img src=…>` embeds).
- **`GET /verify.php`** (also `/verify`, `/api/verify`) — params `api_key`, `order_id`. Returns `{status:"success", data:{utr,transaction_id,…}}` when the API-key owner's Gmail inbox shows a matching credit notification, `{status:"pending"}` while waiting, or `{status:"error"}` on failure.
