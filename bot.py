...
def main_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{ce_button('cart')} BUY HACK", callback_data="user_buy_hack"),
         InlineKeyboardButton(f"{ce_button('disk')} DOWNLOAD APK", callback_data="user_downloads")],
        [InlineKeyboardButton(f"{ce_button('key')} MY KEY", callback_data="user_my_keys_0"),
         InlineKeyboardButton(f"{ce_button('stock')} STOCK", callback_data="user_stock")],
        [InlineKeyboardButton(f"{ce_button('user')} PROFILE", callback_data="user_profile"),
         InlineKeyboardButton(f"{ce_button('mobile')} HOW TO USE", callback_data="user_how_to")],
        [InlineKeyboardButton(f"{ce_button('contact')} SUPPORT", callback_data="user_contact")],
    ])
...
def admin_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{ce_button('stats')} Dashboard", callback_data="admin_stats")],
        [InlineKeyboardButton(f"{ce_button('bag')} Products", callback_data="admin_products"),
         InlineKeyboardButton(f"{ce_button('key')} Keys", callback_data="admin_keys")],
        [InlineKeyboardButton(f"{ce_button('user')} Users", callback_data="admin_users"),
         InlineKeyboardButton(f"{ce_button('pending')} Pending Payments", callback_data="admin_pending_payments")],
        [InlineKeyboardButton(f"{ce_button('broadcast')} Broadcast", callback_data="admin_broadcast"),
         InlineKeyboardButton(f"{ce_button('chat')} Tickets", callback_data="admin_tickets")],
        [InlineKeyboardButton(f"{ce_button('settings')} Settings", callback_data="admin_settings"),
         InlineKeyboardButton(f"{ce_button('tools')} Maintenance", callback_data="adm_maintenance")],
        [InlineKeyboardButton(f"{ce_button('session')} UPI Session", callback_data="admin_svc_session"),
         InlineKeyboardButton(f"{ce_button('shield')} API Keys", callback_data="admin_api_keys")],
        [InlineKeyboardButton(f"{ce_button('disk')} Backup DB", callback_data="adm_export_db")],
        [InlineKeyboardButton(f"{ce_button('back')} Exit Admin", callback_data="user_main")],
    ])
...
def pagination_kb(current_page: int, total_pages: int, prefix: str, back_cb: str):
    ...
    nav = []
    if current_page > 0:
        nav.append(InlineKeyboardButton(f"{ce_button('left')} Prev", callback_data=f"{prefix}_{current_page-1}"))
    nav.append(InlineKeyboardButton(f"{ce_button('memo')} Page {current_page+1}/{total_pages}", callback_data="ignore"))
    if current_page < total_pages - 1:
        nav.append(InlineKeyboardButton(f"Next {ce_button('right')}", callback_data=f"{prefix}_{current_page+1}"))
    if len(nav) > 1:
        buttons.append(nav)
...
async def handle_user_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Buy Hack ───────────────────────────────────────────────────────────
            ...
            buttons = [
                [InlineKeyboardButton(f"{ce_button('game')} {p['name']}", callback_data=f"buy_prod_{p['id']}")]
                for p in products
            ]
            ...
            for pl in plans:
                ...
                if stock > 0:
                    btn_text = f"{ce_button('star')} {pl['duration']} — ₹{pl['price']/100:.2f}"
                    btn_cb = f"buy_plan_{pl['id']}"
                else:
                    btn_text = f"{ce_button('fail')} {pl['duration']} — SOLD OUT"
                    btn_cb = "ignore"
                buttons.append([InlineKeyboardButton(btn_text, callback_data=btn_cb)])
            ...
            buttons = [
                [InlineKeyboardButton(f"{ce_button('pay')} GENERATE PAYMENT QR", callback_data=f"gen_qr_{plan_id}")],
                [InlineKeyboardButton(f"{ce_button('fail')} CANCEL", callback_data=f"buy_prod_{plan['product_id']}")],
            ]
            await safe_edit_text(update, context, text, InlineKeyboardMarkup(buttons))
        ...
        # ── Generate QR via microservice ───────────────────────────────────────
            ...
            buttons = [
                [InlineKeyboardButton(f"{ce_button('success')} I'VE PAID", callback_data=f"verify_pay_{order_id}")],
                [InlineKeyboardButton(f"{ce_button('refresh')} Generate New QR", callback_data=f"gen_qr_{plan_id}")],
                [InlineKeyboardButton(f"{ce_button('back')} Cancel", callback_data=f"buy_plan_{plan_id}")],
            ]
            try:
                await query.message.delete()
            except Exception:
    ...
    try:
        ...
        # ── Public API Keys Panel ──────────────────────────────────────────────
            ...
            if not token:
                await safe_edit_text(
                    update, context,
                    f"<blockquote>{ce('fail')} <b>Login first</b>\n\n"
                    "Open <b>UPI Session</b> and login with your Gmail App "
                    "Password before issuing API keys. Each key uses your "
                    "Gmail inbox to verify payments.</blockquote>",
                    back_kb("admin_main"),
                )
                return
            ...
            try:
                async with aiohttp.ClientSession() as _s:
                    async with _s.get(
                        f"{svc_url}/api/keys/list",
                        headers={"Authorization": f"Bearer {token}"},
                        timeout=aiohttp.ClientTimeout(total=8),
                    ) as _r:
                        _j = await _r.json()
                        if _j.get("status") == "success":
                            keys = _j.get("data", {}).get("keys", [])
                    async with _s.get(
                        f"{svc_url}/health",
                        timeout=aiohttp.ClientTimeout(total=4),
                    ) as _hr:
                        if _hr.status == 200:
                            public_base = (await _hr.json()).get("public_base", svc_url)
            except Exception as e:
                await safe_edit_text(
                    update, context,
                    f"<blockquote>{ce('fail')} <b>Cannot reach payment service.</b>\n"
            ...
            text = (
                f"<blockquote><b>{ce('shield')} PUBLIC API KEYS</b></blockquote>\n\n"
                f"<b>Base URL:</b>\n<code>{public_base}</code>\n"
            ...
            buttons = [[InlineKeyboardButton(
                f"{ce_button('plus')} Generate New API Key",
                callback_data="adm_api_create",
            )]]

            if keys:
                text += "\n"
                for k in keys[:10]:
                    masked = k["api_key"][:14] + "…" + k["api_key"][-4:]
                    flag = ce('success') if k.get("enabled") else ce('fail')
                    text += (
                        f"{flag} <b>{k.get('label', 'default')}</b>\n"
                        f"<code>{k['api_key']}</code>\n"
                        f"  QRs: {k['qr_count']} • Verified: {k['verify_count']}\n"
                        f"  Last used: {k.get('last_used_at_ist', '—')}\n\n"
                    )
                    buttons.append([InlineKeyboardButton(
                        f"{ce_button('fail')} Revoke {k.get('label', '')[:14]} ({masked[-8:]})",
                        callback_data=f"adm_api_revoke_{k['api_key']}",
                    )])
            else:
                text += (
                    f"\n<i>No API keys yet. Generate one to let any bot or "
                    f"website call your payment API.</i>\n\n"
                    f"<b>Endpoints:</b>\n"
                    f"• <code>GET {public_base}/qr.php</code>\n"
                    f"• <code>GET {public_base}/verify.php</code>\n"
                    f"• <code>GET {public_base}/docs</code>\n"
                )

            buttons.append([InlineKeyboardButton(
                f"{ce_button('memo')} View Docs",
                url=f"{public_base}/docs",
            )])
            buttons.append([InlineKeyboardButton(
                f"{ce_button('back')} Back",
                callback_data="admin_main",
            )])
            await safe_edit_text(
                update, context, text, InlineKeyboardMarkup(buttons),
            )

        elif data == "adm_api_create":
            await query.answer("Creating key…", show_alert=False)
            token = db.get_setting("payment_svc_token", "")
            svc_url = db.get_setting("payment_svc_url", "http://localhost:8000")
            if not token:
                await safe_edit_text(
                    update, context,
                    f"<blockquote>{ce('fail')} Login required.</blockquote>",
                    back_kb("admin_main"),
                )
                return
            try:
                async with aiohttp.ClientSession() as _s:
                    async with _s.post(
                        f"{svc_url}/api/keys/create",
                        json={"label": f"key-{int(time.time())}",
                              "payee_name": db.get_setting("payee_name", "Hack Store")},
                        headers={"Authorization": f"Bearer {token}"},
                        timeout=aiohttp.ClientTimeout(total=8),
                    ) as _r:
                        _j = await _r.json()
                ...
                await safe_edit_text(
                    update, context,
                    f"<blockquote>{ce('fail')} Service error: <code>{e}</code></blockquote>",
                    back_kb("admin_api_keys"),
                )
                return
            ...
            if _j.get("status") != "success":
                await safe_edit_text(
                    update, context,
                    f"<blockquote>{ce('fail')} {_j.get('message', 'Create failed.')}</blockquote>",
                    back_kb("admin_api_keys"),
                )
                return

            new_key = _j["data"]["api_key"]
            docs_url = _j["data"].get("docs_url", "")
            text = (
                f"<blockquote><b>{ce('success')} NEW API KEY ISSUED</b></blockquote>\n\n"
                f"<b>Key (tap to copy):</b>\n<code>{new_key}</code>\n\n"
                f"<b>Quick Test:</b>\n"
                f"<code>{docs_url.replace('/docs', '')}/qr.php?api_key={new_key}"
                f"&upi=YOURUPI@bank&amount=10</code>\n\n"
                f"<i>Save this key — share it with your other bots / sites. "
                f"You can revoke it anytime.</i>"
            )
            buttons = [
                [InlineKeyboardButton(f"{ce_button('memo')} Open Docs", url=docs_url)],
                [InlineKeyboardButton(f"{ce_button('back')} Back to Keys",
                                      callback_data="admin_api_keys")],
            ]
            await safe_edit_text(update, context, text, InlineKeyboardMarkup(buttons))

        elif data.startswith("adm_api_revoke_"):
            api_key = data[len("adm_api_revoke_"):]
            await query.answer("Revoking…", show_alert=False)
            token = db.get_setting("payment_svc_token", "")
            svc_url = db.get_setting("payment_svc_url", "http://localhost:8000")
            try:
                async with aiohttp.ClientSession() as _s:
                    async with _s.post(
                        f"{svc_url}/api/keys/revoke",
            ...
            if _j.get("status") not in ("ok", "success"):
                msg = _j.get("message", "Revoke failed.")
                await safe_edit_text(
                    update, context,
                    f"<blockquote>{ce('fail')} {msg}</blockquote>",
                    back_kb("admin_api_keys"),
                )
                return
            ...
            await safe_edit_text(
                update, context,
                f"<blockquote>{ce('success')} <b>API key revoked.</b></blockquote>",
                back_kb("admin_api_keys"),
            )
        ...
        # ── Products ───────────────────────────────────────────────────────────
            ...
            buttons = [[InlineKeyboardButton(f"{ce_button('bag')} {p['name']}", callback_data=f"adm_prod_{p['id']}")] for p in prods]
            ...
            buttons = [
                [InlineKeyboardButton(f"{ce_button('pencil')} Edit Description", callback_data=f"adm_edit_desc_{p_id}")],
                [InlineKeyboardButton(f"{ce_button('loop')} Toggle Status", callback_data=f"adm_ptog_{p_id}")],
                [InlineKeyboardButton(f"{ce_button('bag')} Manage Plans", callback_data=f"adm_plans_{p_id}")],
                [InlineKeyboardButton(f"{ce_button('fail')} Delete Product", callback_data=f"adm_delprod_{p_id}")],
            ...
            buttons = [
                [InlineKeyboardButton(f"{ce_button('fail')} Yes, Delete", callback_data=f"adm_confirm_delprod_{p_id}")],
                [InlineKeyboardButton(f"{ce_button('back')} Cancel", callback_data=f"adm_prod_{p_id}")],
            ]
            await safe_edit_text(update, context, text, InlineKeyboardMarkup(buttons))
